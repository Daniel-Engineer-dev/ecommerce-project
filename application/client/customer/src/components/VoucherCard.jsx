...
const fallbackImage = 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&auto=format&fit=crop&q=80';
  ...
  return (
    <>
      ...
      >
        ...
        <button type="button" className="voucher-card__cart" onClick={handleAddToCart} aria-label="Thêm vào giỏ hàng">
      ...
      <AnimatePresence>
        {showQuickView && (
          ...
          >
            ...
            >
              <button type="button" className="quick-view__close" onClick={() => setShowQuickView(false)} aria-label="Đóng">
                <X size={20} />
              </button>
              <img src={voucher.image_url} alt={voucher.title} onError={handleImageError} />
              <div className="quick-view__content">
                <span className="quick-view__merchant">{voucher.company_name || 'Dealzy Partner'}</span>
                <h2>{voucher.title}</h2>
                <p>{voucher.description || 'Voucher ưu đãi đang được phát hành trên Dealzy.'}</p>
                ...
                <div className="quick-view__price">
                  <strong>{Number(voucher.sale_price).toLocaleString('vi-VN')}đ</strong>
                  <span>{Number(voucher.original_price).toLocaleString('vi-VN')}đ</span>
                </div>
