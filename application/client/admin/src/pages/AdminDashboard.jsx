...
const StatCard = ({ title, value, icon: Icon, change, isPositive, colorClass, bgClass }) => (
  <motion.div
    ...
    className="bg-white p-6 rounded-xl border border-slate-200/60 shadow-sm flex flex-col justify-between"
  >
    <div className="flex justify-between items-start">
      ...
      <div className={`p-3 rounded-lg bg-slate-100/80 text-slate-800`}>
    ...
    <div className="flex items-center gap-1.5 mt-4 text-xs font-bold">
      <span className={`flex items-center gap-0.5 px-2 py-0.5 rounded-md ${
        isPositive ? 'bg-slate-50 text-slate-800' : 'bg-slate-100 text-slate-500'
      }`}>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/60 pb-6">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-xl border border-slate-200/40 self-start md:self-auto shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200/60 shadow-sm lg:col-span-7 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
            ...
            <div className="flex items-center gap-1 text-slate-800 bg-slate-100 px-2.5 py-1 rounded-lg text-xs font-bold">
          ...
          <div className="h-72 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={timeDataMock[timeUnit]} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0f172a', borderRadius: '8px', color: '#fff', border: 'none', fontSize: '12px', fontWeight: '600' }}
                  labelStyle={{ color: '#94a3b8', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="doanhThu" name="Doanh Thu" stroke="#0f172a" strokeWidth={2.5} fillOpacity={1} fill="url(#colorDoanhThu)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200/60 shadow-sm lg:col-span-5 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
            <div>
          ...
          <div className="h-72 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={timeDataMock[timeUnit]} margin={{ top: 10, right: 5, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="customer" name="Khách Hàng" stackId="a" fill="#0f172a" radius={[0, 0, 0, 0]} barSize={24} />
                <Bar dataKey="partner" name="Đối Tác" stackId="a" fill="#94a3b8" radius={[4, 4, 0, 0]} barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
