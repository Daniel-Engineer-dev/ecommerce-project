...
class AuthService {
  ...
  async login(username, password) {
    ...
    if (user.role === "Partner") {
      const partnerRes = await pool.query(
        "SELECT status FROM Partners WHERE user_id = $1",
        [user.user_id],
      );
      if (partnerRes.rows[0].status === "Pending")
        throw new Error("Tài khoản đang chờ xét duyệt");
      if (partnerRes.rows[0].status === "Rejected")
  ...
  async updateProfile(userId, role, data) {
    ...
    try {
      ...
      if (data.phone) {
        ...
        if (phoneExists.rows.length > 0)
          throw new Error(
            "Sá»‘ Ä‘iá»‡n thoáº¡i nÃ y Ä‘Ã£ Ä‘Æ°á»£c sá»­ dá»¥ng",
          );
      }
      await client.query(
        "UPDATE Users SET email = $1, phone = $2 WHERE user_id = $3",
      ...
      if (role === "Partner") {
        ...
        if (data.branches) {
          await client.query("DELETE FROM Branches WHERE partner_id = $1", [
            userId,
          ]);
  ...
  async forgotPassword(data) {
    ...
    if (!user)
      throw new Error(
        email ? "Email không tồn tại" : "Số điện thoại không tồn tại",
      );
    ...
    if (email) {
      ...
      await sendEmail({
        ...
        template: {
          ...
          body: "Nhấp nút bên dưới để thiết lập mật khẩu mới. Liên kết này chỉ có hiệu lực trong 15 phút.",
          buttonText: "Đặt lại mật khẩu",
          buttonUrl: resetUrl,
          footer:
            "Nếu bạn không yêu cầu, hãy bỏ qua email này hoặc liên hệ bộ phận hỗ trợ.",
        },
      });
      ...
      await sendSms({
        phone: user.phone,
        content: `Ma OTP cua ban la: ${resetToken}`,
      });
  ...
  async verifyOtp(phone, otp) {
    ...
    if (
      result.rows[0].reset_token !== tokenHash ||
