...
* **Cách nhóm đã áp dụng:**
  * **Cơ sở dữ liệu:** Thiết lập PostgreSQL thông qua file [scriptDatabase.sql](SUBMISSION_PACKAGE/02_source_code/database_script_and_seed/scriptDatabase.sql) bao gồm 18 bảng nghiệp vụ.
  * **Phân quyền vai trò:** Cột `role` trong bảng `Users` nhận các giá trị `Customer`, `Partner`, `Admin` để định tuyến và phân quyền.
  * **Xác thực phiên làm việc:** Sử dụng JWT token và middleware xác thực vai trò đặt tại [middleware/](application/server/middleware/).
...
| :--- | :--- | :--- | :--- | :--- |
| **BR-CUS-01** | Đăng ký tài khoản bằng Email/SĐT, kiểm tra trùng lặp, OTP mô phỏng. | [CustomerRegistration.jsx](application/client/customer/src/pages/CustomerRegistration.jsx) | [authRoutes.js](application/server/modules/auth/authRoutes.js)<br>[authController.js](application/server/modules/auth/authController.js)<br>[authService.js](application/server/modules/auth/authService.js) | Bảng `Users` và `Customers`. Ràng buộc duy nhất qua chỉ mục partial `idx_users_email_unique_not_null` và `idx_users_phone_unique_not_null`. |
| **BR-CUS-02** | Đăng nhập, đăng xuất, quên mật khẩu, cập nhật hồ sơ cá nhân. | [AuthPage.jsx](application/client/customer/src/pages/AuthPage.jsx)<br>[Profile.jsx](application/client/customer/src/pages/Profile.jsx)<br>[ResetPassword.jsx](application/client/customer/src/pages/ResetPassword.jsx) | [authService.js](application/server/modules/auth/authService.js) (các hàm `login`, `updateProfile`, `forgotPassword`, `resetPassword`). | Hàm băm `bcryptjs` mã hóa mật khẩu trước khi lưu. Cập nhật trực tiếp vào bảng `Customers` và `Users`. |
| **BR-CUS-03** | Tìm kiếm voucher theo từ khóa & Lọc theo danh mục, khu vực, giá, đối tác. | [SearchVouchers.jsx](application/client/customer/src/pages/SearchVouchers.jsx)<br>[Home.jsx](application/client/customer/src/pages/Home.jsx) | [voucherRoutes.js](application/server/modules/shared/voucherRoutes.js)<br>[voucherService.js](application/server/modules/shared/voucherService.js) (`getVouchers`). | Truy vấn bảng `Vouchers` kết hợp `Categories` và `Partners`. Chỉ trả về voucher có `status = 'Approved'`, còn hạn bán và còn tồn kho. |
| **BR-CUS-04** | Xem thông tin chi tiết voucher (giá, điều kiện, chi nhánh, đánh giá). | [VoucherDetail.jsx](application/client/customer/src/pages/VoucherDetail.jsx) | [voucherService.js](application/server/modules/shared/voucherService.js) (`getVoucherById`). | Kết hợp bảng `Vouchers`, `Voucher_Branches`, `Branches`, `Reviews` để lấy toàn bộ dữ liệu liên quan. |
| **BR-CUS-05** | Quản lý giỏ hàng ở phía client (thêm, cập nhật số lượng, xóa). | [Cart.jsx](application/client/customer/src/pages/Cart.jsx) | Sử dụng `localStorage` lưu trữ giỏ hàng phía Client nhằm giảm tải cho server và nâng cao trải nghiệm người dùng. | Không ghi vào DB cho đến khi checkout để tối ưu bộ nhớ. |
| **BR-CUS-06** | Tạo đơn hàng, khai báo thông tin người nhận quà tặng, chọn cổng thanh toán mô phỏng. | [Checkout.jsx](application/client/customer/src/pages/Checkout.jsx) | [orderRoutes.js](application/server/modules/customer/orderRoutes.js)<br>[orderService.js](application/server/modules/customer/orderService.js) (`createOrder`). | Ghi dữ liệu vào bảng `Orders` (lưu `gift_recipient_name`, `gift_recipient_email` nếu là quà tặng) và bảng `Order_Items`. |
| **BR-CUS-07** | Nhận và quản lý ví E-Voucher (mã voucher dạng text, QR mô phỏng, trạng thái sử dụng). | [Profile.jsx](application/client/customer/src/pages/Profile.jsx) (Tab E-Vouchers) | [orderService.js](application/server/modules/customer/orderService.js) (`getCustomerEVouchers`). | Đọc dữ liệu từ bảng `E_Vouchers`. Hiển thị mã QR mô phỏng bằng thư viện sinh mã QR phía Client. |
| **BR-CUS-08** | Đánh giá voucher sau khi mua và gửi phản hồi/khiếu nại về đơn hàng. | [VoucherDetail.jsx](application/client/customer/src/pages/VoucherDetail.jsx) (phần Review)<br>[Profile.jsx](application/client/customer/src/pages/Profile.jsx) (Tab Đơn hàng / Gửi khiếu nại) | [complaintRoutes.js](application/server/modules/customer/complaintRoutes.js)<br>[complaintService.js](application/server/modules/customer/complaintService.js) (`createComplaint`). | **Đánh giá:** Ghi bảng `Reviews` (Ràng buộc bằng Trigger `trg_validate_review` chỉ cho phép review nếu đã mua).<br>**Khiếu nại:** Ghi bảng `Complaints`. |
...
| Mã yêu cầu | Mô tả chi tiết yêu cầu | File giao diện (Frontend) | API & Service xử lý (Backend) | Cơ sở dữ liệu (PostgreSQL) |
| :--- | :--- | :--- | :--- | :--- |
| **BR-PAR-01** | Đăng ký và quản lý thông tin doanh nghiệp, đại diện pháp lý và các chi nhánh. | [PartnerRegistration.jsx](application/client/partner/src/pages/PartnerRegistration.jsx)<br>[App.jsx](application/client/partner/src/App.jsx) (Tab Hồ sơ) | [authService.js](application/server/modules/auth/authService.js) (`registerPartner`) | Ghi thông tin doanh nghiệp vào bảng `Partners` (mặc định trạng thái `Pending` chờ Admin duyệt) và danh sách chi nhánh vào bảng `Branches`. |
| **BR-PAR-02** | Tạo mới chương trình voucher (giá, số lượng phát hành, hạn dùng, chi nhánh áp dụng). | [App.jsx](application/client/partner/src/App.jsx) (Tab Tạo voucher) | [partnerRoutes.js](application/server/modules/partner/partnerRoutes.js)<br>[partnerService.js](application/server/modules/partner/partnerService.js) (`createVoucher`) | Lưu thông tin voucher vào bảng `Vouchers`. Lưu liên kết chi nhánh áp dụng vào bảng nối `Voucher_Branches`. |
| **BR-PAR-03** | Gửi voucher sang trạng thái chờ duyệt và theo dõi kết quả phê duyệt từ Admin. | [App.jsx](application/client/partner/src/App.jsx) (Tab Quản lý voucher) | [partnerService.js](application/server/modules/partner/partnerService.js) (`submitVoucherForApproval`) | Cập nhật cột `status` trong bảng `Vouchers` thành `Pending`. |
| **BR-PAR-04** | Cập nhật thông tin voucher trong phạm vi cho phép, theo dõi lượng tồn và đã bán. | [App.jsx](application/client/partner/src/App.jsx) (Tab Chi tiết voucher) | [partnerService.js](application/server/modules/partner/partnerService.js) (`updateVoucher`) | Thực hiện cập nhật bảng `Vouchers`. Ràng buộc trigger kiểm tra tồn kho không được vượt quá số lượng phát hành. |
| **BR-PAR-05** | Tra cứu thông tin mã voucher code bằng nhập tay hoặc scan mã QR mô phỏng. | [App.jsx](application/client/partner/src/App.jsx) (Tab Xác thực E-Voucher) | [partnerService.js](application/server/modules/partner/partnerService.js) (`verifyEVoucher`) | Đọc thông tin E-Voucher từ bảng `E_Vouchers`, liên kết bảng `Order_Items` và `Vouchers` để kiểm tra thông tin voucher gốc. |
| **BR-PAR-06** | Xác nhận voucher đã sử dụng (`Redeem`) tại chi nhánh cụ thể, cập nhật trạng thái. | [App.jsx](application/client/partner/src/App.jsx) (Nút Xác nhận sử dụng) | [partnerService.js](application/server/modules/partner/partnerService.js) (`redeemEVoucher`) | Cập nhật trạng thái trong bảng `E_Vouchers` thành `Used`, ghi nhận `used_at` và `used_branch_id`. Tích hợp Trigger `trg_validate_voucher_usage` kiểm tra chi nhánh. |
| **BR-PAR-07** | Xem báo cáo dashboard đối tác: doanh thu, số phát hành, số đã bán, tỷ lệ sử dụng. | [App.jsx](application/client/partner/src/App.jsx) (Tab Dashboard) | [partnerService.js](application/server/modules/partner/partnerService.js) (`getDashboardStats`) | Thực hiện gom nhóm dữ liệu (Aggregation queries) từ các bảng `Vouchers`, `Order_Items` và `E_Vouchers` của đối tác. |
...
| Mã yêu cầu | Mô tả chi tiết yêu cầu | File giao diện (Frontend) | API & Service xử lý (Backend) | Cơ sở dữ liệu (PostgreSQL) |
| :--- | :--- | :--- | :--- | :--- |
| **BR-ADM-01** | Quản lý người dùng: Xem, tìm kiếm, lọc và khóa/mở khóa tài khoản Khách hàng & Đối tác. | [UserManagement.jsx](application/client/admin/src/pages/UserManagement.jsx) | [adminRoutes.js](application/server/modules/admin/adminRoutes.js)<br>[adminService.js](application/server/modules/admin/adminService.js) (`getAllUsers`, `toggleUserLock`, `changeUserRole`). | Đọc gộp `Users` + `Customers`/`Partners`; cập nhật cột `is_active` trong bảng `Customers`/`Partners` khi khóa/mở. `changeUserRole` chủ động **chặn** đổi vai trò trực tiếp để giữ toàn vẹn hồ sơ & dữ liệu giao dịch. |
| **BR-ADM-02** | Quản lý đối tác: Duyệt hồ sơ đối tác đang pending, đổi trạng thái hoạt động doanh nghiệp. | [PartnerApproval.jsx](application/client/admin/src/pages/PartnerApproval.jsx) | [adminService.js](application/server/modules/admin/adminService.js) (`getPendingPartners`, `updatePartnerStatus`). | Cập nhật cột `status` trong bảng `Partners` thành `Approved` hoặc `Rejected`. Ghi chú lý do từ chối nếu có. |
| **BR-ADM-03** | Duyệt voucher: Xem danh sách chờ duyệt, duyệt hoặc từ chối kèm lý do, hoặc tạm ẩn voucher. | [VoucherManagement.jsx](application/client/admin/src/pages/VoucherManagement.jsx) | [adminVoucherRoute.js](application/server/modules/admin/Voucher/adminVoucherRoute.js)<br>[adminVoucherService.js](application/server/modules/admin/Voucher/adminVoucherService.js) (`reviewVoucher`). | Cập nhật cột `status` trong bảng `Vouchers` (`Approved`, `Rejected`, `Suspended`). |
| **BR-ADM-04** | Quản lý đơn hàng: Tra cứu đơn hàng, xác nhận thanh toán thủ công, xử lý hoàn tiền. | [OrderManagement.jsx](application/client/admin/src/pages/OrderManagement.jsx) | [adminOrderRoute.js](application/server/modules/admin/Order/adminOrderRoute.js)<br>[adminOrderService.js](application/server/modules/admin/Order/adminOrderService.js) (`confirmPaymentManual`). | Ghi nhận trạng thái `Paid` hoặc `Refunded` trong bảng `Orders`. Tự động gọi hàm hoàn tồn kho và khóa E-Voucher khi hoàn tiền toàn bộ. |
| **BR-ADM-05** | Quản lý nội dung: Chỉnh sửa trang tĩnh có cấu trúc, preview trước khi lưu, lưu vết lịch sử. | [ContentManagement.jsx](application/client/admin/src/pages/ContentManagement.jsx) | [contentRoutes.js](application/server/modules/shared/contentRoutes.js)<br>[adminService.js](application/server/modules/admin/adminService.js) (`updateContentItem`). | Ghi dữ liệu JSON có cấu trúc vào bảng `Content_Items` (cột `data`). Ghi vết thay đổi (before/after) vào bảng `Content_Item_Revisions`. |
| **BR-ADM-06** | Dashboard quản trị: Tổng quan số liệu doanh thu toàn hệ thống, đối tác, đơn hàng. | [AdminDashboard.jsx](application/client/admin/src/pages/AdminDashboard.jsx) | [adminService.js](application/server/modules/admin/adminService.js) (`getDashboardStats`). | Gom nhóm dữ liệu thống kê từ toàn bộ CSDL (`Orders`, `Users`, `Partners`, `Vouchers`). |
| **BR-ADM-07** | Nhật ký hệ thống: Tra cứu và kiểm tra lại thao tác của các tài khoản Admin/Partner. | [SystemLogs.jsx](application/client/admin/src/pages/SystemLogs.jsx) | [adminService.js](application/server/modules/admin/adminService.js) (`getSystemLogs`). | Đọc và lọc dữ liệu từ bảng `System_Logs` (được ghi nhận tự động thông qua hàm trợ giúp `logAction`). |
| **BR-ADM-08** | Phân quyền tài khoản quản trị (RBAC theo phạm vi): `SuperAdmin` tạo admin mới, gán **phạm vi quản trị** (sub-role) và khóa/mở tài khoản admin. Mỗi phân hệ (đối tác, voucher, đơn hàng, khiếu nại, nội dung, nhật ký) chỉ admin đúng phạm vi mới thao tác được. | [UserManagement.jsx](application/client/admin/src/pages/UserManagement.jsx) (panel *Phân quyền Quản trị viên* + modal *Tạo admin*)<br>[SideBar.jsx](application/client/admin/src/components/SideBar.jsx) & [scopes.js](application/client/admin/src/scopes.js) (ẩn menu ngoài phạm vi)<br>[NoAccess.jsx](application/client/admin/src/components/NoAccess.jsx) (chặn truy cập bằng URL) | [adminScope.js](application/server/middleware/adminScope.js) (`requireAdmin`, `requireScope` — chốt chặn **403** ở tầng API)<br>[adminRoutes.js](application/server/modules/admin/adminRoutes.js)<br>[adminService.js](application/server/modules/admin/adminService.js) (`getAdmins`, `createAdmin`, `setAdminScope`, `toggleAdminLock`)<br>[authService.js](application/server/modules/auth/authService.js) & [authMiddleware.js](application/server/middleware/authMiddleware.js) (JWT mang `scope`, chặn admin bị khóa). | Cột `admin_scope` (ràng buộc `CHECK` 6 giá trị: `SuperAdmin`, `PartnerModerator`, `VoucherModerator`, `OrderManager`, `ContentEditor`, `SupportAgent`) và `is_active` trong bảng `Users`. Backfill mọi admin cũ = `SuperAdmin`. Ràng buộc nghiệp vụ: không tự đổi quyền/tự khóa mình, luôn giữ tối thiểu một `SuperAdmin` hoạt động. Mọi thao tác ghi vết qua `logAction` (RB-12). |
...
1. **RB-01 (Duyệt mới được bán):** 
   * Áp dụng tại API tìm kiếm công khai: [voucherService.js](application/server/modules/shared/voucherService.js) chỉ lấy `status = 'Approved'`.
...
5. **RB-05 (Chỉ phát hành E-Voucher khi đã thanh toán thành công - Paid):**
   * E-Voucher chỉ được tạo khi trạng thái đơn hàng chuyển sang `Paid` trong transaction tại hàm `completePayment` của [orderService.js](application/server/modules/customer/orderService.js).
